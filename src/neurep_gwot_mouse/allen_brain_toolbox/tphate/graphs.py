...
class kNNGraph(DataGraph):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if knn > data.shape[0] - 2:
            warnings.warn(
                f"Cannot set knn ({knn}) to be greater than "
                f"n_samples - 2 ({data.shape[0] - 2}). Setting knn={data.shape[0] - 2}"
            )
            knn = data.shape[0] - 2
        if knn_max is not None and knn_max < knn:
            warnings.warn(f"Cannot set knn_max ({knn_max}) to be less than knn ({knn}). Setting knn_max={knn}")
            knn_max = knn
        if n_pca in [None, 0, False] and data.shape[1] > 500:
            warnings.warn(
                f"Building a kNNGraph on data of shape {data.shape} is expensive. Consider setting n_pca.",
                UserWarning,
            )
    ...
    def knn_tree(self):
        ...
        try:
            ...
            try:
                ...
                # invalid metric
                warnings.warn(
                    f"Metric {self.distance} not valid for `sklearn.neighbors.BallTree`. "
    ...
    def _check_duplicates(self, distances, indices):
        if np.any(distances[:, 1] == 0):
            ...
            if np.sum(distances[:, 1:] == 0) < 20:
                ...
                duplicate_names = ", ".join([f"{i[0]} and {i[1]}" for i in duplicate_ids])
                warnings.warn(
                    f"Detected zero distance between samples {duplicate_names}. "
                ...
                warnings.warn(
                    f"Detected zero distance between {np.sum(np.sum(distances[:, 1:] == 0)) // 2} pairs of samples. "
                    "Consider removing duplicates to avoid errors in "
                    "downstream processing.",
                    RuntimeWarning,
                )
    ...
    def build_kernel_to_data(self, Y, knn=None, knn_max=None, bandwidth=None, bandwidth_scale=None):
        ...
        if knn > self.data.shape[0]:
            warnings.warn(
                f"Cannot set knn ({knn}) to be greater than "
                f"n_samples ({self.data_nu.shape[0]}). Setting knn={self.data_nu.shape[0]}"
            )
        ...
        if self.decay is None or self.thresh == 1:
            ...
            with _logger.task("affinities"):
                ...
                _logger.debug(f"search_knn = {search_knn}; {len(update_idx)} remaining")
                if len(update_idx) > 0:
                ...
                while (
                    ...
                    update_idx = [
                        ...
                        if np.max(d) < (radius if isinstance(bandwidth, numbers.Number) else radius[i])
                    ]
                    _logger.debug(f"search_knn = {search_knn}; {len(update_idx)} remaining")
                    # increase the knn search
                    search_knn = min(search_knn * self.search_multiplier, knn_max)
                if search_knn > self.data_nu.shape[0] / 2:
                    knn_tree = NearestNeighbors(n_neighbors=search_knn, algorithm="brute", n_jobs=self.n_jobs).fit(
                        self.data_nu
                    )
                if len(update_idx) > 0:
                    if search_knn == knn_max:
                        _logger.debug(f"knn search to knn_max ({knn_max}) on {len(update_idx)}")
                        ...
                        _logger.debug(f"radius search on {len(update_idx)}")
                        # give up - radius search
                        dist_new, ind_new = knn_tree.radius_neighbors(
                            Y[update_idx, :],
                            radius=radius if isinstance(bandwidth, numbers.Number) else np.max(radius[update_idx]),
                        )
                        for i, idx in enumerate(update_idx):
                            distances[idx] = dist_new[i]
                            indices[idx] = ind_new[i]
...
class LandmarkGraph(DataGraph):
    ...
    def __init__(self, data, n_landmark=2000, n_svd=100, **kwargs):
        ...
        if n_landmark >= data.shape[0]:
            raise ValueError(f"n_landmark ({n_landmark}) >= n_samples ({data.shape[0]}). Use kNNGraph instead")
        if n_svd >= data.shape[0]:
            warnings.warn(
                f"n_svd ({n_svd}) >= n_samples ({data.shape[0]}) Consider using kNNGraph or lower n_svd",
                RuntimeWarning,
            )
    ...
    def extend_to_data(self, data, **kwargs):
        ...
        if sparse.issparse(kernel):
            pnm = sparse.hstack(
                [sparse.csr_matrix(kernel[:, self.clusters == i].sum(axis=1)) for i in np.unique(self.clusters)]
            )
        else:
            pnm = np.array(
                [np.sum(kernel[:, self.clusters == i], axis=1).T for i in np.unique(self.clusters)]
            ).transpose()
        pnm = normalize(pnm, norm="l1", axis=1)
        return pnm
...
class TraditionalGraph(DataGraph):
        ...
        **kwargs,
        ...
        if precomputed is not None:
            if precomputed not in ["distance", "affinity", "adjacency"]:
                raise ValueError(
                    f"Precomputed value {precomputed} not recognized. Choose from ['distance', 'affinity', 'adjacency']"
                )
            elif data.shape[0] != data.shape[1]:
                raise ValueError(f"Precomputed {precomputed} must be a square matrix. {data.shape} was given")
            elif (data < 0).sum() > 0:
                raise ValueError(f"Precomputed {precomputed} should be non-negative")
        self.knn = knn
        self.decay = decay
        self.bandwidth = bandwidth
        self.bandwidth_scale = bandwidth_scale
    ...
    def build_kernel(self):
                ...
                if self.precomputed == "distance":
                    ...
                    if np.any(pdx == 0):
                        ...
                        if len(duplicate_ids) < 20:
                            ...
                            warnings.warn(
                                f"Detected zero distance between {len(duplicate_ids)} pairs of samples. "
                                "Consider removing duplicates to avoid errors in "
                                "downstream processing.",
                                RuntimeWarning,
                            )
                    ...
                    raise ValueError(
                        f"precomputed='{self.precomputed}' not recognized. "
        ...
        if sparse.issparse(K):
            if not (
                isinstance(K, sparse.csr_matrix) or isinstance(K, sparse.csc_matrix) or isinstance(K, sparse.bsr_matrix)
            ):
                K = K.tocsr()
            K.data[K.data < self.thresh] = 0
            K = K.tocoo()
...
class MNNGraph(DataGraph):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if sample_idx is None:
            ...
            raise ValueError(f"sample_idx ({len(sample_idx)}) must be the same length as data ({data.shape[0]})")
    ...
    def _check_symmetrization(self, kernel_symm, theta):
        if (
            ...
            raise TypeError(f"Expected `theta` as a float. Got {type(theta)}.")
        else:
    ...
    def set_params(self, **params):
        ...
        for arg in knn_kernel_args:
            if arg in params and params[arg] != getattr(self, arg):
                raise ValueError(f"Cannot update {arg}. Please create a new graph")
    ...
    def build_kernel(self):
        ...
        with _logger.task("subgraphs"):
            ...
            for i, idx in enumerate(self.samples):
                _logger.debug(f"subgraph {i}: sample {idx}, n = {np.sum(self.sample_idx == idx)}, knn = {self.knn}")
        ...
        with _logger.task("MNN kernel"):
            ...
            for i, X in enumerate(self.subgraphs):
                ...
                for j, Y in enumerate(self.subgraphs):
                    if i == j:
                        continue
                    with _logger.task(f"kernel from sample {self.samples[i]} to {self.samples[j]}"):
