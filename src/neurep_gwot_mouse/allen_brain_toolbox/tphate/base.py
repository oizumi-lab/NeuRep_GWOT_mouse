...
class Data(Base):
    ...
    def _parse_n_pca_threshold(self, data, n_pca, rank_threshold):
        ...
        if isinstance(n_pca, numbers.Number):
            if not float(n_pca).is_integer():  # cast it to integer
                ...
                warnings.warn(
                    f"Cannot perform PCA to fractional {n_pca} dimensions. Rounding to {n_pcaR}",
                    RuntimeWarning,
                )
                n_pca = n_pcaR
            ...
            if n_pca < 0:
                ...
                warnings.warn(
                    f"Cannot perform PCA to {n_pca} dimensions on "
                    f"data with min(n_samples, n_features) = {np.min(data.shape)}",
                    RuntimeWarning,
                )
                n_pca = 0
        ...
        if n_pca in [0, False, None]:  # cast 0, False to None.
            ...
            n_pca = "auto"
            _logger.info("Estimating n_pca from matrix rank. Supply an integer n_pca for fixed amount.")
        ...
        if rank_threshold is not None and n_pca != "auto":
            warnings.warn(
                f"n_pca = {n_pca}, therefore rank_threshold of {rank_threshold} "
    ...
    def _check_data(self, data):
        if len(data.shape) != 2:
            msg = f"Expected 2D array, got {len(data.shape)}D array instead (shape: {data.shape}.) "
            if len(data.shape) < 2:
                msg += (
        ...
        if self.n_pca is not None and (self.n_pca == "auto" or self.n_pca < self.data.shape[1]):
            with _logger.task("PCA"):
                ...
                if self.n_pca == "auto":
                    ...
                    if self.n_pca == 0:
                        raise ValueError(
                            f"Supplied threshold {threshold} was greater than "
                            f"maximum singular value {smax} "
                            "for the data matrix"
                        )
                    _logger.info(f"Using rank estimate of {self.n_pca} as n_pca")
    ...
    def transform(self, Y):
        ...
        try:
            ...
            # shape is wrong
            raise ValueError(
                f"data of shape {Y.shape} cannot be transformed"
                f" to graph built on data of shape {self.data.shape}. "
                f"Expected shape ({Y.shape[0]}, {self.data.shape[1]})"
            )
    ...
    def inverse_transform(self, Y, columns=None):
        ...
        try:
            ...
            raise ValueError(
                f"data of shape {Y.shape} cannot be inverse transformed"
                f" from graph built on reduced data of shape ({self.data_nu.shape[0]}, {self.data_nu.shape[1]}). Expected shape ({Y.shape[0]}, {self.data_nu.shape[1]})"
            )
...
class BaseGraph(with_metaclass(abc.ABCMeta, Base)):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if gamma is not None:
            warnings.warn(f"gamma is deprecated. Setting theta={gamma}", FutureWarning)
            theta = gamma
        ...
        if not (isinstance(anisotropy, numbers.Real) and 0 <= anisotropy <= 1):
            raise ValueError(f"Expected 0 <= anisotropy <= 1. Got {anisotropy}")
    ...
    def _check_symmetrization(self, kernel_symm, theta):
        if kernel_symm not in ["+", "*", "mnn", None]:
            raise ValueError(f"kernel_symm '{kernel_symm}' not recognized. Choose from '+', '*', 'mnn', or 'none'.")
        elif kernel_symm != "mnn" and theta is not None:
            warnings.warn(f"kernel_symm='{kernel_symm}' but theta is not None. Setting kernel_symm='mnn'.")
            self.kernel_symm = kernel_symm = "mnn"

        if kernel_symm == "mnn":
            if theta is None:
                self.theta = theta = 1
                warnings.warn(f"kernel_symm='mnn' but theta not given. Defaulting to theta={self.theta}.")
            elif not isinstance(theta, numbers.Number) or theta < 0 or theta > 1:
                raise ValueError(f"theta {theta} not recognized. Expected a float between 0 and 1")
    ...
    def symmetrize_kernel(self, K):
        ...
        if self.kernel_symm == "+":
            ...
            _logger.debug(f"Using mnn symmetrization (theta = {self.theta}).")
    ...
    def to_pygsp(self, **kwargs):
        ...
        if "precomputed" in kwargs:
            if kwargs["precomputed"] != "affinity":
                warnings.warn(
                    "Cannot build PyGSPGraph with precomputed={}. Using 'affinity' instead.".format(
                        kwargs["precomputed"]
                    ),
                    UserWarning,
                )
        ...
        if "use_pygsp" in kwargs:
            if kwargs["use_pygsp"] is not True:
                warnings.warn(
                    "Cannot build PyGSPGraph with use_pygsp={}. Use True instead.".format(kwargs["use_pygsp"]),
                    UserWarning,
                )
    ...
    def to_pickle(self, path):
        ...
        is_oldpygsp = all([isinstance(self, pygsp.graphs.Graph), int(sys.version.split(".")[1]) < 7])
        if is_oldpygsp:
    ...
    def shortest_path(self, method="auto", distance=None):
        ...
        if distance == "constant":
            ...
            D.data = np.sqrt(np.sum((self.data_nu[D.row] - self.data_nu[D.col]) ** 2, axis=1))
        elif distance == "affinity":
            D = sparse.csr_matrix(self.K)
            D.data = -1 * np.log(D.data)
        else:
            raise ValueError(f"Expected `distance` in ['constant', 'data', 'affinity']. Got {distance}")
...
class DataGraph(with_metaclass(abc.ABCMeta, Data, BaseGraph)):
    ...
    def _check_extension_shape(self, Y):
        ...
        """
        if len(Y.shape) != 2:
            raise ValueError(f"Expected a 2D matrix. Y has shape {Y.shape}")
        if not Y.shape[1] == self.data_nu.shape[1]:
            ...
            if Y.shape[1] == self.data.shape[1]:
                ...
                if self.data.shape[1] != self.data_nu.shape[1]:
                    # PCA is possible
                    msg = f"Y must be of shape either (n, {self.data.shape[1]}) or (n, {self.data_nu.shape[1]})"
                else:
                    # no PCA, only one choice of shape
                    msg = f"Y must be of shape (n, {self.data.shape[1]})"
                raise ValueError(msg)
        return Y
