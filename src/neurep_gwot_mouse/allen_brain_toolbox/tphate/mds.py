...
def classic(D, n_components=2, random_state=None):
    ...
    """
    _logger.debug(f"Performing classic MDS on {type(D).__name__} of shape {D.shape}...")
    D = D**2
...
def sgd(D, n_components=2, random_state=None, init=None):
    ...
    _logger.debug(f"Performing SGD MDS on {type(D)} of shape {D.shape}...")
    N = D.shape[0]
    D = squareform(D)
...
def smacof(
    ...
    Y : array-like, shape=[n_samples, n_components]
        embedded data
    """
    _logger.debug(f"Performing non-metric MDS on {type(D)} of shape {D.shape}...")
...
def embed_MDS(
    ...
    if how not in ["classic", "metric", "nonmetric"]:
        raise ValueError(f"Allowable 'how' values for MDS: 'classic', 'metric', or 'nonmetric'. '{how}' was passed.")
    if solver not in ["sgd", "smacof"]:
        raise ValueError(f"Allowable 'solver' values for MDS: 'sgd' or 'smacof'. '{solver}' was passed.")
