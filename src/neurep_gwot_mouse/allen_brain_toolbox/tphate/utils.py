...
def check_greater(x, **params):
    ...
    for p in params:
        if not isinstance(params[p], numbers.Number) or params[p] <= x:
            raise ValueError(f"Expected {p} > {x}, got {params[p]}")
...
def check_in(choices, **params):
    ...
    for p in params:
        if params[p] not in choices:
            raise ValueError(f"{p} value {params[p]} not recognized. Choose from {choices}")
...
def check_between(v_min, v_max, **params):
    ...
    for p in params:
        if params[p] < v_min or params[p] > v_max:
            raise ValueError(f"Expected {p} between {v_min} and {v_max}, got {params[p]}")
...
@deprecated(version="1.5.0", reason="Use graphtools.matrix.sparse_nonzero_discrete instead")
def sparse_nonzero_discrete(*args, **kwargs):
    return matrix.sparse_nonzero_discrete(*args, **kwargs)


@deprecated(version="1.5.0", reason="Use graphtools.matrix.dense_nonzero_discrete instead")
...
@deprecated(version="1.5.0", reason="Use graphtools.matrix.matrix_is_equivalent instead")
