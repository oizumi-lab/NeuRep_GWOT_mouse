...
def Graph(
    ...
    **kwargs : extra arguments for `pygsp.graphs.Graph`
    ...
    if graphtype == "knn":
        ...
        raise ValueError(f"graphtype '{graphtype}' not recognized. Choose from ['knn', 'mnn', 'exact', 'auto']")

    # set add landmarks if necessary
    parent_classes = [basegraph]
    msg = f"Building {graphtype} graph"
    ...
    try:
        ...
        raise RuntimeError(f"unknown graph classes {parent_classes}")
    ...
    _logger.debug(
        "Initializing {} with arguments {}".format(
            parent_classes,
            ", ".join([f"{key}='{value}'" for key, value in params.items() if key != "data"]),
        )
    )
    return Graph(**params)
...
def from_igraph(G, attribute="weight", **kwargs):
    ...
    if "precomputed" in kwargs:
        if kwargs["precomputed"] != "adjacency":
            warnings.warn(
                "Cannot build graph from igraph with precomputed={}. Use 'adjacency' instead.".format(
                    kwargs["precomputed"]
                ),
                UserWarning,
            )
    ...
    try:
        ...
        if str(e) == "Attribute does not exist":
            warnings.warn(
                f"Edge attribute {attribute} not found. Returning unweighted graph",
                UserWarning,
            )
