# %% [markdown]
# ##  Evaluation of alignment results for 1on1
...
# # Usage
# ```python ta_paper_ind_evaluation.py 'stimulus_name' 'exp_name'``` <br />
# * Evaluates the results of experiments performed with `ta_paper_ind_alignment.py`. <br />
...
def aggregate_calcuration(results_dir: Path, v_name: str = "spearman_r") -> np.ndarray:
    ...
    """
    # get n_samples
    n = sum(1 for _ in results_dir.glob("VISp_vs_VISp/*"))
    ...
    for i, area_a in enumerate(areas):
        for j, area_b in enumerate(areas):
            ...
            # get values
            for k, p in enumerate(results_dir.glob(f"{area_a}_vs_{area_b}/*")):

                # load values
                if v_name == "spearman_r":
                    with open(p / "partial_RSA.json", "r") as f:
                        d = json.load(f)
                    matrix[k, i, j] = next(iter(d.values()))

                elif v_name == "ot_top1_matching_rate":
                    matrix[k, i, j] = np.load(p / "OT_matching_rate.npy")
...
# %%
def parse_args():
    parser = argparse.ArgumentParser(description="Individual alignment experiment evaluator")
...
if __name__ == "__main__":
    "usage: python ta_paper_ind_evaluation.py 'stimulus_name' 'exp_name'"
