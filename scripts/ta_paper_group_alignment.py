#%% [markdown]
# # Analysis between pseudo-mice with different numbers of individuals <br>
# - NormalizeScaler -> MeanTrials -> MakeRDM(metric="cosine")
# - EntropicGW2Computation

# # Usage
# ```python ta_paper_group_alignment.py <setting_file.csv>``` <br>
# <br>
# For how to create the setting file, refer to dummy_group_alignment_setting.csv. <br>
...
# Third Party Library
import pandas as pd
from neurep_gwot_mouse.alignment.ta_paper_group import group_alignment_experiment
...
# session_split_dir = current_dir / f"../session_split"
# config_dir = current_dir / f"../config"
# ################################################################################################

def parse_args():
    parser = argparse.ArgumentParser(description="Group alignment experiment runner")
    parser.add_argument("setting_file", type=str, help="Path to the setting CSV file")
    parser.add_argument("--target_dir", type=str, default=None, help="Directory to save results")
    parser.add_argument("--whole_data_dir", type=str, default=None, help="Directory containing source data")
    parser.add_argument("--session_split_dir", type=str, default=None, help="Directory for session split files")
...
def main():
    """usage:
    ```
    python ta_paper_group_alignment.py <setting_file.csv> --target_dir <target_dir> --whole_data_dir <whole_data_dir> --session_split_dir <session_split_dir> --config_dir <config_dir>
    ```
    """
    args = parse_args()
    current_dir = Path(__file__).parent
    ...
    session_split_dir = Path(args.session_split_dir) if args.session_split_dir else current_dir / "../session_split"
    ...
    for i in range(len(exp_setting_df)):
        ...
        group_alignment_experiment(
            ...
            current_dir=target_dir / exp_setting.loc["stimulus_name"],
            data_dir=whole_data_dir / exp_setting.loc["stimulus_name"],
            session_split_path=session_split_dir / exp_setting.loc["session_split_name"],
